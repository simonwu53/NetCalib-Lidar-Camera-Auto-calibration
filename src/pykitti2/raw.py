...
import pykitti2.utils as utils
...
class raw:
    ...
    def get_depth_len(self):
        """get the number of frames for annotated depth image"""
        return len(self.cam2d_files)

    @property
    def cam2d(self):
        """Generator to read image files for cam2 depth maps (left)."""
        return utils.yield_images(self.cam2d_files, mode='I')

    def get_cam2d(self, idx):
        """Read image file for cam2 depth map (left) at the specified index."""
        return utils.load_image(self.cam2d_files[idx], mode='I')

    @property
    def cam3d(self):
        """Generator to read image files for cam3 depth maps (right)."""
        return utils.yield_images(self.cam3d_files, mode='I')

    def get_cam3d(self, idx):
        """Read image file for cam3 depth map (right) at the specified index."""
        return utils.load_image(self.cam3d_files[idx], mode='I')
    ...
    def _get_file_lists(self):
        ...
        self.velo_files = sorted(glob.glob(
            os.path.join(self.data_path, 'velodyne_points',
                         'data', '*.bin')))
        # Shan's custom codes
        self.cam2d_files = sorted(glob.glob(
            os.path.join(self.data_path, 'proj_depth', 'groundtruth', 'image_02', '*.{}'.format(self.imtype))
        ))
        self.cam3d_files = sorted(glob.glob(
            os.path.join(self.data_path, 'proj_depth', 'groundtruth', 'image_03', '*.{}'.format(self.imtype))
        ))
