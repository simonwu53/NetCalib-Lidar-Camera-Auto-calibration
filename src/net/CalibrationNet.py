...
def disparity_matching_module_2():
    # this module is built for Lidar & Cameras disparity maps respectively
    model = nn.Sequential(
        # layer 1, conv+NIN, out 32, 128, 384
        Conv2d(in_c=1, out_c=32, kerner_size=3, stride=1, padding=1, batch_norm=True, activation='relu'),
        Conv2d(in_c=32, out_c=16, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        Conv2d(in_c=16, out_c=16, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        nn.MaxPool2d(kernel_size=2),

        # layer 2, out 128, 64, 192
        Conv2d(in_c=16, out_c=64, kerner_size=3, stride=1, padding=1, batch_norm=True, activation='relu'),
        Conv2d(in_c=64, out_c=48, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        Conv2d(in_c=48, out_c=32, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        nn.MaxPool2d(kernel_size=2),

        # layer 3, out 128, 32, 96
        Conv2d(in_c=32, out_c=96, kerner_size=3, stride=1, padding=1, batch_norm=True, activation='relu'),
        Conv2d(in_c=96, out_c=96, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        Conv2d(in_c=96, out_c=96, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        nn.MaxPool2d(kernel_size=2),
    )
    return model
...
def global_regression_module_2():
    # this module is built for concatenated lidar & camera tensor
    model = nn.Sequential(
        # layer 1, conv+NIN, out N, 256, 16, 48
        Conv2d(in_c=192, out_c=256, kerner_size=3, stride=1, padding=1, batch_norm=True, activation='relu'),
        Conv2d(in_c=256, out_c=192, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        Conv2d(in_c=192, out_c=192, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        Conv2d(in_c=192, out_c=320, kerner_size=3, stride=1, padding=1, batch_norm=True, activation='relu'),
        Conv2d(in_c=320, out_c=256, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        Conv2d(in_c=256, out_c=256, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        nn.MaxPool2d(kernel_size=2),

        # layer 2, out N, 512, 8, 24
        Conv2d(in_c=256, out_c=384, kerner_size=3, stride=1, padding=1, batch_norm=True, activation='relu'),
        Conv2d(in_c=384, out_c=384, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        Conv2d(in_c=384, out_c=384, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        Conv2d(in_c=384, out_c=512, kerner_size=3, stride=1, padding=1, batch_norm=True, activation='relu'),
        Conv2d(in_c=512, out_c=448, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        Conv2d(in_c=448, out_c=448, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        nn.MaxPool2d(kernel_size=2),

        # layer 3, out N, 64, 4, 12
        Conv2d(in_c=448, out_c=1024, kerner_size=3, stride=1, padding=1, batch_norm=True, activation='relu'),
        Conv2d(in_c=1024, out_c=512, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        Conv2d(in_c=512, out_c=256, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        Conv2d(in_c=256, out_c=512, kerner_size=3, stride=1, padding=1, batch_norm=True, activation='relu'),
        Conv2d(in_c=512, out_c=256, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        Conv2d(in_c=256, out_c=128, kerner_size=1, stride=1, padding=0, batch_norm=True, activation='relu'),
        nn.MaxPool2d(kernel_size=2),
...
class CalibNet2(nn.Module):
    def __init__(self):
        super(CalibNet2, self).__init__()
        self.disp_matching_L = disparity_matching_module_2()
        self.disp_matching_C = disparity_matching_module_2()
        self.global_regression = global_regression_module_2()
        self.FC = nn.Linear(in_features=2688, out_features=1024)
        self.rot_output = nn.Linear(in_features=1024, out_features=3)
        self.trans_output = nn.Linear(in_features=1024, out_features=3)
        return
    ...
    def forward(self, disp_l, disp_c):
        ...
        # global regression, out 1, 64, 21
        regressed = self.global_regression(concat).squeeze(0).view(1, -1)

        # output, out 1, 3
        logits = self.FC(regressed)
        rot = self.rot_output(logits)
        trans = self.trans_output(logits)
...
if __name__ == '__main__':
    device = 'cuda'
    if device == 'cuda':
        x = torch.rand((1, 1, 256, 768)).cuda()
        x2 = torch.rand((1, 1, 256, 768)).cuda()
    else:
        x = torch.rand((1, 1, 256, 768))
        x2 = torch.rand((1, 1, 256, 768))
